...
import com.juxun.business.street.adapter.CommissionAdapter;
...
/**
 * 
 * 类名称：CommissionAnalysisAcitivity 类描述：分佣分析 首页 创建人：罗富贵 创建时间：2016年5月10日
 * 
 * @version
 * 
 */
public class CommissionAnalysisAcitivity extends BaseActivity {
	...
	private void initValues() {
		...
		initPull();
		commissionAdapter = new CommissionAdapter(this, analysisbeans);
		mListView.setAdapter(commissionAdapter);
		obtainData();
		loading();
	...
	public void clickMethod(View v) {
		if (v.getId() == R.id.rbtn_zhou) {
			...
			birth = new SelectBirthdayPop(CommissionAnalysisAcitivity.this,
...
//		map.put("admin_id", storeBean.getAdmin_id() + "");
		...
		map.put("pageSize", 10 + "");
		mQueue.add(ParamTools.packParam(Constants.commissionAnalysis, this, this,
				map));
		loading();
	}
	...
	public void onResponse(String response, String url) {
		...
		try {
			...
			if (stauts == 0) {
				...
				commissionAdapter.updateListView(analysisbeans);
