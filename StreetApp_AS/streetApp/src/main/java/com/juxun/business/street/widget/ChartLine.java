...
/**
 * Created by TianBin on 2017/7/1 12:38. Description :仿金融APP收益曲线图
 */
...
public class ChartLine extends View implements View.OnTouchListener {
     ...
     * 数据集合
     */
    private List<ShopStaModel> datas = new ArrayList<ShopStaModel>();
     ...
     * 选中的value
     */
    private ShopStaModel selectedValue;
    ...
    private void init(Context context) {
        ...
        linePaint.setStrokeWidth(TypedValue.applyDimension(
                TypedValue.COMPLEX_UNIT_DIP, 3f, getResources()
                        .getDisplayMetrics()));
        linePaint.setColor(getResources().getColor(R.color.back_blue));
        ...
        fillPaint.setStrokeWidth(TypedValue.applyDimension(
                TypedValue.COMPLEX_UNIT_DIP, 2f, getResources()
                        .getDisplayMetrics()));
        fillPaint.setColor(getResources().getColor(R.color.back_blue));
        ...
        pointPaint.setStrokeWidth(TypedValue.applyDimension(
                TypedValue.COMPLEX_UNIT_DIP, 3f, getResources()
                        .getDisplayMetrics()));
        pointPaint.setColor(getResources().getColor(R.color.blue));
        ...
        xSeparatePaint.setStrokeWidth(TypedValue.applyDimension(
                TypedValue.COMPLEX_UNIT_DIP, 0.5f, getResources()
                        .getDisplayMetrics()));
        xSeparatePaint.setColor(getResources().getColor(R.color.back_d8));

        labelXPaint.setAntiAlias(true);
        labelXPaint.setColor(Color.BLACK);
        labelXPaint.setTextAlign(Paint.Align.CENTER);
        labelXPaint.setTextSize(TypedValue.applyDimension(
                TypedValue.COMPLEX_UNIT_SP, 12, getResources()
                        .getDisplayMetrics()));
        labelXHeight = labelXPaint.getFontMetrics().bottom
                - labelXPaint.getFontMetrics().top;

        labelYPaint.setAntiAlias(true);
        labelYPaint.setColor(Color.BLACK);
        labelYPaint.setTextAlign(Paint.Align.RIGHT);
        labelYPaint.setTextSize(TypedValue.applyDimension(
                TypedValue.COMPLEX_UNIT_SP, 12, getResources()
                        .getDisplayMetrics()));
        labelYHeight = labelYPaint.getFontMetrics().bottom
                - labelYPaint.getFontMetrics().top;

        labelValuePaint.setAntiAlias(true);
        labelValuePaint.setColor(Color.BLUE);
        labelValuePaint.setTextAlign(Paint.Align.CENTER);
        labelValuePaint.setTextSize(TypedValue.applyDimension(
        ...
        labelValueHeight = labelValuePaint.getFontMetrics().bottom
                - labelValuePaint.getFontMetrics().top;

    }

    public void setDatas(List<ShopStaModel> datas) {
         ...
         * Y轴的处理: 最大最小两个边界数值
         */
        min = (int) (datas.get(0).count);// 放大为整数，避免浮点运算
        max = (int) (datas.get(0).count);
        ...
        int count = 1;
        while (max > topNum) {
            topNum = (count + 1) * 50; // 第一次小于50就绕过去，大于50进来了，
            count++;
        }
        count = 1;
        max = topNum * 100; // 一次递增50: 最大值的变化 50,100,150
        min = 0;
        ...
        perHeight = (getHeight() - getPaddingTop() - getPaddingBottom())
                / (ySize + 1);

        // 坐标右对齐========Y轴处理
        String str = negativePos == -1 ? datas.get(datas.size() - 1).count
                + "" : datas.get(negativePos).count + "";
        xVertical = getPaddingLeft() + labelYPaint.measureText(str);
        xPadding = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 10,
                getResources().getDisplayMetrics());

        // X轴处理和曲线绘制,居中对齐
        eachWidth = (getWidth() - getPaddingRight() - xVertical - xPadding)
                / xSize;
        bottomY = getHeight() - getPaddingBottom() - perHeight - labelYHeight
                / 3f;// 最低点坐标
        ...
        for (int i = 0; i < datas.size(); i++) {
            ...
            float y = bottomY - perY * (datas.get(i).count - min / 100f);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < ySize; i++) {
            ...
            // 绘制横向分割线
            canvas.drawLine(xVertical + xPadding, y, getWidth()
                    - getPaddingRight(), y, xSeparatePaint);
        }
        ...
        for (int i = 0; i < xSize; i++) {
            ...
            canvas.drawText(label, x,
                    getHeight() + Tools.dip2px(getContext(), 10)
                            - getPaddingBottom() - labelXHeight * 1.8f,
                    labelXPaint);
        }
        ...
        if (playAnim) {
            ...
            if (fillAreaHasAnim) {
                float currX = (points[datas.size() - 1][0] - points[0][0])
                        * (1 - mAnimatorValue) + points[0][0];
                if (isFillArea) {
            ...
            if (isOver) { // 动画结束的情况下画点
                float pointRadius = TypedValue.applyDimension(
                        TypedValue.COMPLEX_UNIT_DIP, 3, getResources()
                                .getDisplayMetrics());
                if (drawPoints) {
                    for (float[] f : points) {
                        canvas.drawCircle(f[0], f[1], pointRadius + 5,
                                pointPaint);
                        canvas.drawCircle(f[0], f[1], pointRadius, whitePaint);
                    }
                }
            }
        }
...
//			 canvas.drawBitmap(bg,lastX-bg.getWidth(),lastY-bg.getHeight(),new
//			 Paint());
            // String tip=10;
            //canvas.drawText("",lastX-labelValuePaint.measureText/2f,lastY-labelValueHeight/2f,labelValuePaint);
            ...
            if (drawPoints) {
                for (float[] f : points) {
                    canvas.drawCircle(f[0], f[1], pointRadius + 5, pointPaint);
                    canvas.drawCircle(f[0], f[1], pointRadius, whitePaint);
                }
            }
        }
    ...
    public ChartLine setFillArea(boolean fillArea) {
        isFillArea = fillArea;
        return this;
    }

    public ChartLine setPlayAnim(boolean playAnim) {
        this.playAnim = playAnim;
        return this;
    }

    public ChartLine setDrawPoints(boolean drawPoints) {
        this.drawPoints = drawPoints;
        return this;
    }

    public ChartLine setFillAreaHasAnim(boolean fillAreaHasAnim) {
    ...
    public boolean onTouch(View v, MotionEvent event) {
        if (datas != null && datas.size() > 0) {
            if (isInArea(event.getX(), event.getY())) {
                // Toast.makeText(getContext(),df.format(selectedValue.percent)+"%",
    ...
    private boolean isInArea(float x, float y) {
        ...
        for (int i = 0; i < points.length; i++) {
            if ((x <= points[i][0] + radius) && x >= (points[i][0] - radius)
                    && (y <= points[i][1] + radius)
                    && y >= (points[i][1] - radius)) {
