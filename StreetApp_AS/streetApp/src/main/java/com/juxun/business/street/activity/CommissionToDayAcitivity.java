...
import com.juxun.business.street.adapter.CommissionToDayAdapter;
...
/**
 * 
 * 类名称：CommissionToDayAcitivity 类描述：当日 分佣分析 首页 创建人：罗富贵 创建时间：2016年5月10日
 * 
 * @version
 * 
 */
public class CommissionToDayAcitivity extends BaseActivity {
	...
	private CommissionToDayAdapter paidAdapter;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_today_commission);
		ViewUtils.inject(this);
		initTitle();
		title.setText("当日分佣详情");
		initValues();
	}
	...
	private void initValues() {
		initPull();
		paidAdapter = new CommissionToDayAdapter(this, analysisbeans);
	...
	public void obtainData() {
		...
		map.put("pageSize", "10");
		mQueue.add(ParamTools.packParam(Constants.commissionWithDay, this, this, map));
		loading();
	}
		...
		try {
			...
			if (stauts == 0) {
				ArrayList<Analysisbean> finance = (ArrayList<Analysisbean>) JSON
						.parseArray(json.getString("analysis_list"), Analysisbean.class);
				if (pagNumber > 1) {
