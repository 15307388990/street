...
public class StringUtil {
	...
	public static boolean checkString(String str) {
		return (null != str && !TextUtils.isEmpty(str) && !str.equalsIgnoreCase("null"));
	}
	...
	public static String toFenByYuan(String moneyStr) {
		...
		if (StringUtil.checkString(moneyStr) && checkMoneyValid(moneyStr)) {
			try {
				return (new BigDecimal(moneyStr).multiply(new BigDecimal("100")).setScale(0, BigDecimal.ROUND_FLOOR)
						+ "");
			} catch (NumberFormatException e) {
				e.printStackTrace();
				return ((Integer.parseInt(moneyStr)) * 100) + "";
	...
	public static boolean checkCreditNum(String num) {
		...
		try {
			if (StringUtil.checkString(num) && !StringUtil.isZeroOnTopOfSrting(num)) {
				if (num.length() >= 15 && num.length() <= 19 && !StringUtil.isZeroOnTopOfSrting(num)) {
					result = true;
				}
			}
		} catch (Exception e) {
	...
	public static String replaceUserNameWithStar(String userName) {
		try {
			...
			String star = "";
			int userNameLen = (userName.length() % 2 == 0) ? (userName.length()) : (userName.length() - 1);
