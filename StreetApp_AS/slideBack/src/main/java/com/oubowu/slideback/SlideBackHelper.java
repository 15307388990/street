...
public class SlideBackHelper {
	 ...
	 */
	public static SlideBackLayout attach(final Activity curActivity,
			final ActivityHelper helper, final SlideConfig config,
		...
		slideBackLayout = new SlideBackLayout(curActivity, contentView,
				preContentView[0], preDecorViewDrawable, config,
				new OnInternalStateListener() {
					...
					public void onClose(Boolean finishActivity) {
						...
						if (config != null && config.isRotateScreen()) {
							...
							if (preActivity[0] != null
									&& preContentView[0].getParent() != getDecorView(preActivity[0])) {
								// Log.e("TAG", ((SlideBackLayout)
								// preContentView[0].getParent()).getTestName()
								// + "这里把欠人的布局放回到上个Activity");
								preContentView[0].setX(0);
								((ViewGroup) preContentView[0].getParent())
										.removeView(preContentView[0]);
								getDecorView(preActivity[0]).addView(
										preContentView[0], 0);
							}
							// else {
						...
						if (finishActivity != null && finishActivity) {
							curActivity.finish();
							curActivity.overridePendingTransition(0,
									R.anim.anim_out_none);
					...
					@Override
					public void onCheckPreActivity(
						...
						// Log.e("TAG", "SlideBackHelper-120行-onFocus(): " +
						// preActivity[0] + ";" + activity);
						if (activity != preActivity[0]) {
							// Log.e("TAG",
							// "SlideBackHelper-122行-onFocus(): 上个Activity变了");
							preActivity[0] = activity;
							preContentView[0] = getContentView(preActivity[0]);
							slideBackLayout
									.updatePreContentView(preContentView[0]);
						}
					}
