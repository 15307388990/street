...
public class SlideBackLayout extends FrameLayout {
	...
	public SlideBackLayout(Context context, View contentView,
			View preContentView, Drawable preDecorViewDrawable,
			SlideConfig config,
	...
	private void initConfig(SlideConfig config) {
		...
		if (mRotateScreen) {
			mContentView.findViewById(android.R.id.content).setOnClickListener(
					new OnClickListener() {
						@Override
	...
	private void addPreContentView() {
		if (mPreContentView.getParent() != SlideBackLayout.this) {
			...
			((ViewGroup) mPreContentView.getParent())
		...
		public void onViewDragStateChanged(int state) {
			...
			case ViewDragHelper.STATE_IDLE:
				if (mContentView.getLeft() == 0) {
					...
					if (mRotateScreen
							&& mCacheDrawView.getVisibility() == INVISIBLE) {
						...
						mCloseFlagForDetached = true;
						// Log.e("TAG", mTestName +
						// ": 滑动到尽头了这个界面要死了，把preContentView给回上个Activity");
		...
		public void onViewPositionChanged(View changedView, int left, int top,
			...
			if (!mRotateScreen && mCacheDrawView.getVisibility() == INVISIBLE) {
				...
				if (!mCheckPreContentView) {
					...
					mOnInternalStateListener
							.onCheckPreActivity(SlideBackLayout.this);
				}
			...
			if (mRotateScreen) {
				// // Log.e("TAG", "滑动上个页面");
				mPreContentView.setX(-mScreenWidth / 2 + percent
						* (mScreenWidth / 2));
			} else {
				mCacheDrawView.setX(-mScreenWidth / 2 + percent
						* (mScreenWidth / 2));
			}
	...
	public void onWindowFocusChanged(boolean hasWindowFocus) {
		...
		// // Log.e("TAG", "onWindowFocusChanged(): " + this + " ; " +
		// hasWindowFocus);
		...
		if (hasWindowFocus) {
			...
			// Log.e("TAG", "SlideBackLayout-378行-onWindowFocusChanged(): " +
			// hasWindowFocus);
			 ...
			 * else if (mRotateScreen && mPreContentView.getParent() !=
			 * SlideBackLayout.this) { // Log.e("TAG", mTestName +
			 * ": 从其他Activity返回来了 "); }
			 */
		} else {
			if (mRotateScreen) {
				...
				if (mCloseFlagForWindowFocus) {
					mCloseFlagForWindowFocus = false;
					// Log.e("TAG", mTestName +
					// ": onWindowFocusChanged前已经调了关闭");
				} else {
					// Log.e("TAG", mTestName +
					// ": 跳转到另外一个Activity，取这个Activity的contentView前把preContentView给回上个Activity");
	...
	protected void onDetachedFromWindow() {
		...
		// // Log.e("TAG", "SlideBackLayout-345行-onDetachedFromWindow(): " +
		// this);
		if (mRotateScreen) {
			// 1.旋转屏幕的时候必调此方法，这里掉onClose目的是把preContentView给回上个Activity
			if (mCloseFlagForDetached) {
				mCloseFlagForDetached = false;
				// Log.e("TAG", mTestName + ": onDetachedFromWindow(): " +
				// "已经调了关闭");
			} else {
				if (getTag() != null
						&& getTag().equals("notScreenOrientationChange")) {
	...
	protected void onConfigurationChanged(Configuration newConfig) {
		...
		mScreenWidth = getResources().getDisplayMetrics().widthPixels;
		// Log.e("TAG", mTestName +
		// ": SlideBackLayout-338行-onConfigurationChanged(): " + mScreenWidth);
